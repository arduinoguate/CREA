    ...
    $.ajax({
      ...
      complete: function(resp) {
        ...
        $.each(json.modulos, function(i, item) {
          ...
          info += '<div class="col-md-3"><a href="'+item.url_libreria+'" target="_blank" class="btn btn-primary">Download</a><br/>';
          info += '</div></div><hr/>';
          $("#files").append(info);
        });
      },
      error: function(jqXHR, textStatus, errorThrown) {
        if (jqXHR.status != '422') {
          ...
          $("#files").html("<center><h1>No libraries available</h1><br/><small>Soon we will be adding ne libraries</small></center>");
      },
    });
  }
